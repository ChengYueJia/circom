...
fn remove_anonymous_from_statement(
    ...
    match stm {
        Statement::MultSubstitution { meta, lhe, op, rhe } => {
            ...
            let subs =
                Statement::MultSubstitution { meta: meta.clone(), lhe: lhe, op: op, rhe: new_rhe };
            if substitutions.is_empty() {
                Result::Ok((subs, comp_declarations, Vec::new(), Vec::new()))
            } else {
                substitutions.push(subs);
                Result::Ok((
                    Statement::Block { meta: meta, stmts: substitutions },
                    comp_declarations,
                    Vec::new(),
                    Vec::new(),
                ))
        ...
        Statement::While { meta, cond, stmt } => {
            ...
            Result::Ok((
                Statement::While { meta: meta, cond: cond, stmt: b_while },
                comp_dec,
                var_declarations,
                subs_out,
            ))
